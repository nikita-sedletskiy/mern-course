...
router.post(
    ...
    [
        ...
        check('password', 'Min length of password is 6 letters')
            .isLength({ min: 6 })
    ],
    async (req, res) => {
    try{
    ...
    async (req, res) => {
        try{
            ...
            if (!errors.isEmpty()) {
                return res.status(400).json({
                    errors: errors.array(),
                    message: 'Incorrect data on login in system'
                })
            }
            ...
            if (!User){
                return res.status(400).json({ message: 'User not founded, try again' })
            }
